...
export default function Signup() {
  ...
  const handleSignup = async () => {
    ...
    if (!email || !password || !username) {
      Alert.alert(
        "Error",
        "Please fill in all required fields (Email, Password, and Username).",
      );
      return;
    ...
    try {
      ...
      const user = userCredential.user; // 2. Update the Firebase User Profile (Display Name/Username)
      ...
      console.log("User created and profile saved:", user.uid); // Redirect to the home screen after successful sign-up
      router.replace("/(tabs)/home");
    } catch (error) {
